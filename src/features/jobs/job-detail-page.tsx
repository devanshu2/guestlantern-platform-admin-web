  ...
  return (
    <div className="space-y-4">
      <section className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-muted">Provisioning workflow</p>
          <h1 className="break-all text-2xl font-semibold text-ink">Job {jobId}</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
            Job detail combines worker lease metadata, ordered provisioning steps, final runtime receipt, and audit events.
          </p>
        </div>
        <Button type="button" variant="secondary" onClick={reload} icon={<RefreshCcw aria-hidden className="h-4 w-4" />}>
          Refresh
        </Button>
      </section>
      ...
      {job ? (
        <>
          ...
          >
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Link className="text-sm font-medium text-brand hover:underline" href={`/restaurants/${job.tenant_id}`}>
                  Open restaurant summary
                </Link>
              </div>
              <ProgressBar value={progress} label="Step progress" />
              {job.error_message ? <Alert tone="danger" title="Backend failure detail">{job.error_message}</Alert> : null}
              <KeyValue
                items={[
          ...
          <Panel title="Provisioning steps" description="Steps are displayed in backend execution order. Error text is shown only when the backend returns it.">
                ...
                <tbody>
                  {job.steps.map((step) => (
                    <tr key={step.step_key}>
                      ...
                      <td className="border-b border-line px-3 py-3 font-mono text-xs">{step.step_key}</td>
                      <td className="border-b border-line px-3 py-3"><StatusBadge status={step.step_status} /></td>
                      <td className="border-b border-line px-3 py-3 text-xs text-muted">{formatDateTime(step.started_at)}</td>
                      <td className="border-b border-line px-3 py-3 text-xs text-muted">{formatDateTime(step.finished_at)}</td>
                      <td className="border-b border-line px-3 py-3 text-xs text-danger">{step.error_message ?? ""}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {job.receipt ? (
            <Panel title="Runtime receipt" description="Final runtime resources returned after successful provisioning. Secret values are not exposed, only secret refs.">
              <KeyValue
                items={[
                  ...
                  { label: "Garage access key ref", value: job.receipt.garage_access_key_id_secret_ref },
                  { label: "Garage secret key ref", value: job.receipt.garage_secret_access_key_secret_ref },
                  { label: "Dragonfly admin ref", value: job.receipt.dragonfly_admin_user_secret_ref },
                  { label: "Dragonfly client ref", value: job.receipt.dragonfly_client_user_secret_ref }
                ]}
              />
            </Panel>
          ) : null}
      ...
      <Panel title="Audit timeline" description="Job-scoped audit events are returned newest first by `/timeline`.">
        {timeline.data ? (
          <ol className="space-y-3">
            {timeline.data.audit_events.map((event) => (
              <li key={event.audit_id} className="rounded-md border border-line bg-slate-50 p-3">
                ...
                <p className="mt-1 text-xs text-muted">Actor: {event.actor_email ?? "system or unavailable"}</p>
              </li>
            ))}
          </ol>
        ) : (
